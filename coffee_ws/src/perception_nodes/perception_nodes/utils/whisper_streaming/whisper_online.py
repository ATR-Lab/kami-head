...
class ASRBase:
    ...
    def __init__(self, lan, modelsize=None, cache_dir=None, model_dir=None, device_type=None, compute_type=None, logfile=sys.stderr):
...
#        logging.getLogger("faster_whisper").setLevel(logger.level)
        ...
        # Get device type and compute type from the instance if they exist
        device_type = getattr(self, 'device_type', 'cuda')
        compute_type = getattr(self, 'compute_type', 'float16')
        
        logger.info(f"Creating WhisperModel with device={device_type}, compute_type={compute_type}")
        model = WhisperModel(
            model_size_or_path, 
            device=device_type, 
...
def asr_factory(args, logfile=sys.stderr):
    ...
    if backend == "openai-api":
        ...
        # Get device and compute type parameters if available
        ...
        logger.info(f"Using device={device_type}, compute_type={compute_type}")
